...
const Question1bii = () => {
  ...
  const [areasFor1bii, setAreasFor1bii] = useState<string[]>([]);

  useEffect(() => {
    // This ensures that `useSearchParams` is only used on the client side
    const pageData = searchParams.get("pageData");
    if (pageData) {
      try {
        const parsedData = JSON.parse(pageData);
        setAreasFor1bii(parsedData);
      } catch (error) {
        console.error("Error parsing pageData:", error);
      }
    }
  }, [searchParams]);
  ...
  const handleSelection = (area: string, option: { text: string; score: number }) => {
    setResponses(prev => ({ ...prev, [area]: option }));
    if (error) {
      setError(""); // Clear error message if any option is selected
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log("1b.ii Responses with Scores:", responseObject);
    ...
    fetch("/api/saveResponses", {
      ...
      .then(data => {
        console.log("Responses saved successfully:", data);
        router.push("/survey/dataCollection/question1c"); // Navigate to next page
      })
      .catch(err => {
        console.error("Error saving responses:", err);
      });
