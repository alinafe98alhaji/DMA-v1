...
const Question1aiii = () => {
  ...
  // State for parsed areas and responses
  const [areasFor1aiii, setAreasFor1aiii] = useState<string[]>([]);
  ...
  // Parse query string on initial render
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const parsedAreasFor1aiii = JSON.parse(params.get("areasFor1aiii") || "[]");
    const parsedAreasFor1av = JSON.parse(params.get("areasFor1av") || "[]");

    setAreasFor1aiii(parsedAreasFor1aiii);
    setAreasFor1av(parsedAreasFor1av);
  }, []);
  ...
  // Validate that all areas have responses
  const validateResponses = () => {
    return areasFor1aiii.every(area => responses[area]);
  };
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      // Determine areas for the next steps
      const areasFor1aiv = areasFor1aiii.filter(area =>
