...
const Question1aiii = () => {
  ...
  // Initialize the areas when the component mounts
  useEffect(
    () => {
      setIsClient(true); // Client-side flag
      const areas1aiii = JSON.parse(searchParams.get("areasFor1aiii") || "[]");
      const areas1av = JSON.parse(searchParams.get("areasFor1av") || "[]");
      setAreasFor1aiii(areas1aiii);
      setAreasFor1av(areas1av);
    },
    [searchParams]
  );

  // If not client-side yet, return null or loading spinner
