...
const DashboardLanding = () => {
  ...
  // Generate or retrieve a completion ID for this session
  const generateCompletionId = () => {
    // Generate a unique ID for the session (or use your custom logic)
    return `completion${Math.floor(Math.random() * 1000)}`; // Example completion ID
  };

  const navigateToFirstQuestion = () => {
    const completionId = generateCompletionId(); // Get or generate a unique completion ID

    // Store the completionId in sessionStorage
