  ...
  try {
    ...
    for (const user of users) {
      ...
      const summedScores = Object.keys(aggregatedScores).map(area => ({
        area,
        totalScore: aggregatedScores[area] / 11 * 100 // Assuming the divisor is 10, adjust as needed
      }));
...
//         response: string;
//         score: number;
//         submittedAt: string; // Add timestamp as well
//       }[];
//     }[];
//     error?: string;
//   };
...
//       // Fetch the responses for the current user from the "data openness and flow" collection
//       const responses = await db
//         .collection("data openess and flow") // Adjust collection name as needed
//         .find(
//           { "responses.score": { $exists: true }, userId: userId },
...
//       // Adjust the calculation as needed based on the collection (e.g., divide by 11)
//       const summedScores = Object.keys(aggregatedScores).map(area => ({
//         area,
//         totalScore: aggregatedScores[area].totalScore / 11 * 100, // Adjust as necessary
