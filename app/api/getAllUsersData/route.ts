...
//         response: string;
//         score: number;
//         submittedAt: string; // Include timestamp
//       }[];
//     }[];
//     error?: string;
//   };
...
//       // Fetch the responses for the current user from the "responses" collection
//       const responses = await db
//         .collection("responses")
//         .find(
//           { "responses.score": { $exists: true }, userId: userId },
...
//       // Calculate the aggregated scores and store full response data for the current user
...
//       // Calculate the percentage for each area (divide by 13 and multiply by 100)
//       const summedScores = Object.keys(aggregatedScores).map(area => ({
//         area,
//         totalScore: aggregatedScores[area].totalScore / 13 * 100, // Dividing by 13 and multiplying by 100
//         responses: aggregatedScores[area].responses // Include all the individual responses
//       }));
