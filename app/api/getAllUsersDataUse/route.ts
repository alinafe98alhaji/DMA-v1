  ...
  try {
    ...
    for (const user of users) {
      ...
      const summedScores = Object.keys(aggregatedScores).map(area => ({
        area,
        totalScore: aggregatedScores[area] / 10 * 100 // Assuming the divisor is 10, adjust as needed
      }));
...
//       // Fetch the responses for the current user from the "data use" collection
//       const responses = await db
//         .collection("data use") // Adjust collection name as needed
//         .find(
//           { "responses.score": { $exists: true }, userId: userId },
...
//       const summedScores = Object.keys(aggregatedScores).map(area => ({
//         area,
//         totalScore: aggregatedScores[area].totalScore / 10 * 100, // Adjust divisor as necessary
